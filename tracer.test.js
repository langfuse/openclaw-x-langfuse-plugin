...
const resolvers = {
  resolveContent: () => ({ input: "the question", output: "the final answer", sessionInput: "the question" }),
  resolveToolIO: () => ({ tc1: { name: "web_search", input: '{"q":"x"}', output: "search results", isError: false } }),
  resolveTurns: () => [{ output: "I'll look that up." }, { output: "the final answer" }],
};

test("a full turn builds one trace, one generation per model call", () => {
  const t = fakeTracing();
...
test("each generation gets its OWN output (not all calls joined together)", () => {
  const t = fakeTracing();
  const { feed } = makeEngine(t, resolvers);
  feed(runSequence());
...
test("cumulative tokens/cost land on the LAST generation", () => {
  const t = fakeTracing();
  const { feed } = makeEngine(t, resolvers);
  feed(runSequence());
  const gens = t.gens();
...
test("context.assembled carries a readable size summary", () => {
  const t = fakeTracing();
  const { feed } = makeEngine(t, resolvers);
  feed(runSequence());
...
test("late async model.call/tool events still nest in the one run trace", () => {
  ...
  feed([...seq.filter((e) => !isAsync(e)), ...seq.filter(isAsync)]);
